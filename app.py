...
with app.app_context():
    ...
    try:
        ...
        try:
            _cur.execute("PRAGMA table_info(offer)")
            offer_cols = [r[1] for r in _cur.fetchall()]
            if 'expires_at' not in offer_cols and offer_cols:
                _cur.execute("ALTER TABLE offer ADD COLUMN expires_at DATETIME")
                print("✅ Added offer.expires_at")
        except Exception as _oe:
            print(f"Offer migration note: {_oe}")
