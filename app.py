...
# Database config - Railway PostgreSQL ya local SQLite
_db_url = os.environ.get('DATABASE_URL', 'sqlite:///database.db')
# Railway deta hai postgres:// but SQLAlchemy chahta hai postgresql://
...
class SiteSettings(db.Model):
    ...
    maintenance_message = db.Column(db.String(500), default='We are currently under maintenance. Please check back soon.')
...
class Offer(db.Model):
    ...
    expires_at = db.Column(db.DateTime, nullable=True)  # exact expiry time
...
@app.route('/login')
def login_page():
    # Maintenance check - admin can still login
    if is_maintenance():
        s = SiteSettings.query.first()
        msg = s.maintenance_message if s else 'Under maintenance.'
...
def home_page(user):
    if user.role == 'admin':
        return redirect(url_for('admin_page'))
    # Maintenance check - admin bypasses
    if is_maintenance():
        s = SiteSettings.query.first()
        msg = s.maintenance_message if s else 'Under maintenance.'
        return render_template('maintenance.html', message=msg)
...
def is_maintenance():
    try:
        s = SiteSettings.query.first()
        return s and s.maintenance_mode
    except:
        return False

@app.route('/api/public/maintenance', methods=['GET'])
def api_maintenance_status():
    try:
        s = SiteSettings.query.first()
        if s and s.maintenance_mode:
            return jsonify({'maintenance': True, 'message': s.maintenance_message}), 200
...
@admin_required
def admin_toggle_maintenance(admin):
    data = request.get_json(silent=True) or {}
    s = SiteSettings.query.first()
    if not s:
        s = SiteSettings()
        db.session.add(s)
    s.maintenance_mode = data.get('enabled', False)
    s.maintenance_message = data.get('message', 'We are currently under maintenance. Please check back soon.')
    s.updated_at = datetime.utcnow()
    db.session.commit()
    return jsonify({'success': True, 'maintenance': s.maintenance_mode}), 200
...
with app.app_context():
    ...
    # Universal migration using SQLAlchemy - works on SQLite AND PostgreSQL
    try:
        ...
        if 'group' in inspector.get_table_names():
            existing = [c['name'] for c in inspector.get_columns('group')]
            group_migrations = [
                ("started_at", "TIMESTAMP"),
                ("auto_glory_enabled", "BOOLEAN DEFAULT FALSE"),
                ("auto_glory_interval", "INTEGER DEFAULT 5"),
                ("auto_glory_amount", "INTEGER DEFAULT 100"),
                ("auto_glory_last_run", "TIMESTAMP"),
            ]
            for col, coltype in group_migrations:
                if col not in existing:
                    try:
                        db.session.execute(text(f'ALTER TABLE "group" ADD COLUMN {col} {coltype}'))
                        db.session.commit()
        ...
        if 'transaction' in inspector.get_table_names():
            existing = [c['name'] for c in inspector.get_columns('transaction')]
            tx_migrations = [
                ("utr_id", "VARCHAR(100)"),
            ...
            for col, coltype in tx_migrations:
                if col not in existing:
                    try:
                        db.session.execute(text(f'ALTER TABLE "transaction" ADD COLUMN {col} {coltype}'))
                        db.session.commit()
                        print(f"✅ Added transaction.{col}")
                    except Exception as _ce:
                        db.session.rollback()

        # Offer table migrations
        if 'offer' in inspector.get_table_names():
            existing = [c['name'] for c in inspector.get_columns('offer')]
            if 'expires_at' not in existing:
                try:
                    db.session.execute(text('ALTER TABLE offer ADD COLUMN expires_at TIMESTAMP'))
                    db.session.commit()
                    print("✅ Added offer.expires_at")
