...
def api_register():
    ...
    if not email:
        email = f"{username}@ffglory.local"
    ...
    if not re.match(r'^[a-z0-9_]+$', username):
        return jsonify({'error': 'Username: lowercase letters, numbers, underscore only'}), 400
...
def api_login():
    data = request.get_json()
    # Accept both 'email' and 'username' field from frontend
